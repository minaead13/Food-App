...
struct NetworkService {
    ...
    private func request<T: Decodable>(route: Route,
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let data = data {
                result = .success(data)
                let responeString = String(data: data, encoding: .utf8) ?? "Could not StringIfy our data"
               // print("The response is: \(responeString)")
    ...
    /// - Parameter :
    ///  - route:  the path that the resource in the backend
    ///  - method:  type of request to be made
    ///  - parameter:  whatever extra info you need to pass to the backend
    /// - Returns : URLRequest
